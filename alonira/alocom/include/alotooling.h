...
typedef struct {
	...
	unsigned long addresses[ALO_TOOLING_DEPTH];
	const char* files[ALO_TOOLING_DEPTH];
} alo_tooling_stack_t;

extern alo_tooling_stack_t alo_tooling_call_stack;

extern void alo_tooling_stack_push(const char* const restrict frame, const unsigned long address, const char* const restrict file);
...
#define ALO_FRAME_BEGIN(function) \
	...
	alo_tooling_stack_push(__func__, (unsigned long) function, __FILE__)
