...
gen_error_t* alo_vga_put_string_at_colored(const char* const restrict string, const gen_size_t length, const alo_vga_color_t foreground, const alo_vga_color_t background, const alo_vga_dimension_t x, const alo_vga_dimension_t y) {
	GEN_TOOLING_AUTO gen_error_t* error = gen_tooling_push(GEN_FUNCTION_NAME, (void*) alo_vga_put_string_at_colored, GEN_FILE_NAME);
	if(error) return error;
